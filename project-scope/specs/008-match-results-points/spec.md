...
### User Story 3 - Regional results identify the top-N per group for Circuit progression (Priority: P1)

Entered group-stage results at Regional identify the top-N players/teams per group (N per the Stage Group Configuration's advancer-count, spec 006 — same mechanism as Grassroots→Regional), who are Circuit-progression candidates for the admin to advance directly. There is no knockout bracket at Regional.

**Why this priority**: This is a hard prerequisite for any Circuit-stage participation — without it, Circuit stays permanently empty. It reuses the exact same group-standings computation as Grassroots (User Story 2), just against a different stage's groups and cutoff.

**Independent Test**: Can be tested by entering a full set of Regional group-stage results and confirming the top-N per group (per the configured advancer-count) are correctly identified as Circuit-progression candidates.

**Acceptance Scenarios**:

1. **Given** all group-stage results are entered for a Regional group, **When** standings are computed, **Then** exactly the top-N ranked participants of that group (per the configured advancer-count) are identified as Circuit-progression candidates (or all of them, if the group has fewer entrants than N — see spec 006 edge cases).

---

### User Story 3b - Circuit/Finals knockout-round results determine bracket progression (Priority: P1)

Entered results for a Circuit or Finals knockout-bracket round determine which participants advance to the next round of that same bracket (winner advances, loser is eliminated from that stage). This is structurally different from Grassroots/Regional group standings — there's no group ranking, just per-fixture winner advancement within the bracket.

**Why this priority**: This is the result-entry mechanism for the two capacity-capped, geography-unrestricted stages (spec 006 User Story 5) — a hard prerequisite for Finals ever having entrants, since Finals is only reachable via the Circuit bracket (plus the separate Cuevo Points-based capacity admission).

**Independent Test**: Can be tested by entering results for every fixture in a knockout round and confirming the winners are correctly identified as the pool the admin draws the next round's pairings from, independent of the Cuevo Points entered for the same matches (User Story 5).

**Acceptance Scenarios**:

1. **Given** all fixtures in a Circuit-bracket round have recorded results, **When** the admin prepares the next round, **Then** the winners of the completed round are the candidates available to pair into the next round's fixtures (the pairing itself is still a manual admin action, per spec 007's manual draw-entry model).
2. **Given** a knockout-bracket fixture result, **When** it is entered, **Then** it does not, by itself, produce a Cuevo Points entry — points are a separate, explicit admin action (User Story 5).
...
- **Group-standing and top-N ties** [Resolved — see FR-013]: an explicit, ordered tiebreaker cascade now governs both Grassroots and Regional group ranking and their respective top-N (per spec 006's Stage Group Configuration) cutoffs.
...
- **FR-002**: The system MUST use entered results to determine group standings at the Grassroots stage (no knockout at this stage) and identify the top-N players/teams per group (N per spec 006's Stage Group Configuration advancer-count), providing the basis for progression decisions to Regional.
- **FR-003**: The system MUST use entered results to determine group standings at the Regional stage (no knockout at this stage either) and identify the top-N players/teams per group (N per spec 006's Stage Group Configuration advancer-count), providing the basis for progression decisions to Circuit.
- **FR-004**: The system MUST use entered knockout-bracket results at the Circuit and Finals stages to determine the winner of each fixture, whose participant advances within that same bracket to the next round (loser is eliminated from that stage's bracket).
...
- **FR-013**: The system MUST apply a defined, ordered tiebreaker cascade — (1) match wins, (2) head-to-head result between the tied participants, (3) frame/rack differential (frames won minus frames lost), (4) total frames won, (5) admin manual resolution as a last resort — when computing Grassroots and Regional group standings and identifying each stage's top-N cutoff, so that ranking and advancement are deterministic.
- **FR-014**: For Circuit/Finals knockout-bracket fixtures, the system MUST record a winner per fixture (FR-001), independent of and not automatically producing a Cuevo Points entry — points at those stages are always a separate, explicit admin action (FR-005).
...
- **Group Standing** *(computed, not stored as raw input, Grassroots/Regional only)*: A per-participant, per-group ranking derived from that group's match results, ordered by the FR-013 tiebreaker cascade; the source for each stage's top-N advancement cutoff (spec 006 Stage Group Configuration).
- **Bracket Progression** *(computed, not stored as raw input, Circuit/Finals only)*: Per-fixture winner advancement within a stage+category's Knockout Bracket (spec 006) — no group ranking involved, just the winner of each round's fixtures becoming available for the next round's pairing.
...
- **SC-003**: The top-N-per-group identification at both Grassroots and Regional matches the expected set in 100% of test cases, including the edge case of a group with fewer entrants than the configured advancer-count N.
...
- **SC-007**: Group standings and top-N cutoffs computed from a test dataset containing intentional ties match the FR-013 tiebreaker cascade's expected output in 100% of test cases.
