...
All buttons: `px-4 py-2.5 text-sm font-medium` at base, `focus-visible:ring-2 ring-cue-500 ring-offset-2` for keyboard focus (never remove focus rings). **Disabled**: `disabled:opacity-50 disabled:cursor-not-allowed disabled:pointer-events-none` on any variant — never convey "disabled" through color/opacity alone without the `disabled` attribute itself, so assistive tech announces it correctly.

### 6.2 Forms & Validation

- Inputs: `rounded-sm border-ink-300 focus:border-cue-500 focus:ring-cue-500`. Placeholder text: `placeholder:text-ink-500` (not `ink-400` — see §2.2). **Disabled inputs**: `disabled:bg-ink-100 disabled:text-ink-500 disabled:cursor-not-allowed`.
...
- **Profile picture / file uploads** (registration, spec 003): show upload progress inline (LiveView's native upload progress), and fall back to an initials-based avatar (`bg-cue-100 text-cue-900`, first-and-last-initial) anywhere a profile picture is referenced but not yet set — never a broken image icon.
