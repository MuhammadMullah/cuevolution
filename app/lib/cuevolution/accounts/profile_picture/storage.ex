defmodule Cuevolution.Accounts.ProfilePicture.Storage do
  ...
  credentials needed. `GCS` (production) uploads to a private Google Cloud
  Storage bucket — see `Storage.GCS` for why a Mix release's own priv/static is the
  wrong place for this in production, and why the bucket is private
  (signed URLs) rather than public.
  ...
  a short-lived signed URL for `GCS`. Never persist the result of
  `url/1` — only the `key` (see `ProfilePicture.store/2`), since a
  signed URL expires and a plain path doesn't survive a redeploy.
  """
